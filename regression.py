# Import necessary libraries
import numpy as np                  # For numerical operations
import pandas as pd                 # For data manipulation
from cassandra.cluster import Cluster  # To connect and query the Cassandra database

# Import the GradientBoostingRegressor from scikit-learn for regression using boosted trees
from sklearn.ensemble import GradientBoostingRegressor

def fetch_data_from_cassandra():
    """
    Connect to the Cassandra database and fetch 70 rows of student data.

    Expected columns in the 'students' table:
        - attendance: Numeric value indicating attendance rate or days attended.
        - prev_gpa: The student's previous year's GPA.
        - dropout: Continuous value representing dropout risk or another continuous metric.

    Returns:
        A pandas DataFrame containing the fetched data.
    """
    # Connect to the Cassandra cluster.
    # Replace '127.0.0.1' with your Cassandra node's IP address if needed.
    cluster = Cluster(['127.0.0.1'])
    
    # Connect to the keyspace named 'school_data'.
    session = cluster.connect('school_data')
    
    # Define the CQL query to select the necessary columns from the 'students' table.
    # LIMIT 70 ensures we only retrieve 70 rows.
    query = "SELECT attendance, prev_gpa, dropout FROM students LIMIT 70"
    rows = session.execute(query)
    
    # Convert the Cassandra rows into a list of dictionaries.
    data = []
    for row in rows:
        data.append({
            'attendance': row.attendance,
            'prev_gpa': row.prev_gpa,
            'dropout': row.dropout  # This column is assumed to be continuous now
        })
    
    # Shutdown the session and cluster connection once the data is fetched.
    session.shutdown()
    cluster.shutdown()
    
    # Create a pandas DataFrame from the list for easier manipulation.
    df = pd.DataFrame(data)
    return df

def train_regression_model(df):
    """
    Train a Gradient Boosting Regressor (boosted trees) using student data.

    The regressor uses the following features:
        - attendance
        - prev_gpa
    to predict the continuous dropout value.

    Parameters:
        df (pandas.DataFrame): DataFrame containing the columns 'attendance', 'prev_gpa', and 'dropout'.

    Returns:
        model (GradientBoostingRegressor): The trained Gradient Boosting regressor.
    """
    # Extract features (attendance and previous GPA) as a NumPy array.
    X = df[['attendance', 'prev_gpa']].values

    # Extract the continuous target variable (dropout).
    y = df['dropout'].values

    # Create and train the Gradient Boosting Regressor.
    # n_estimators specifies the number of boosting stages, and random_state ensures reproducibility.
    model = GradientBoostingRegressor(n_estimators=100, random_state=42)
    model.fit(X, y)

    # OPTIONAL: Evaluate the model's performance on the training data.
    train_score = model.score(X, y)
    print(f"Training R^2 score: {train_score:.2f}")

    # Return the trained model.
    return model

def predict_dropout_value(model, new_data):
    """
    Predict the continuous dropout value for new student data using the trained Gradient Boosting Regressor.

    Parameters:
        model (GradientBoostingRegressor): The trained regression model.
        new_data (list or array): New data points where each point is a list of two features: [attendance, prev_gpa].

    Returns:
        predictions (list): Predicted continuous dropout values for each new data point.
    """
    # Convert new_data into a NumPy array in case it isn't already.
    new_data = np.array(new_data)

    # Use the trained model to predict dropout values.
    predictions = model.predict(new_data)

    # Return the predictions.
    return predictions

# Main execution starts here.
if __name__ == "__main__":
    # -------------------------------------------------------------------------
    # Step 1: Fetch data from the Cassandra database.
    # -------------------------------------------------------------------------
    print("Fetching data from Cassandra database...")
    df = fetch_data_from_cassandra()
    print("Data fetched successfully!")
    print("Sample data:")
    print(df.head())  # Display the first few rows to verify the data.
    
    # -------------------------------------------------------------------------
    # Step 2: Train the Gradient Boosting Regressor using the fetched data.
    # -------------------------------------------------------------------------
    print("\nTraining Gradient Boosting regressor (Boosted Trees) for regression...")
    regression_model = train_regression_model(df)
    print("Training complete!")
    
    # -------------------------------------------------------------------------
    # Step 3: Use the trained model to predict dropout values for new students.
    # -------------------------------------------------------------------------
    # Example new student data:
    # Each sub-list represents [attendance, prev_gpa].
    new_students = [
        [90, 3.5],  # Student with high attendance and a strong GPA.
        [60, 2.0],  # Student with lower attendance and a lower GPA.
        [80, 3.0]   # Student with moderate attendance and GPA.
    ]
    
    print("\nPredicting continuous dropout values for new student data...")
    dropout_predictions = predict_dropout_value(regression_model, new_students)
    
    # Display the predictions in a user-friendly format.
    for i, prediction in enumerate(dropout_predictions):
        print(f"Student {i+1}: Predicted dropout value - {prediction:.2f}")

