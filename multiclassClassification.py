...
# Import scikit-learn modules for our neural network model, scaling, and pipeline creation
...
def fetch_data_from_cassandra():
    ...
    Expected columns in the 'students' table:
        ...
        - dropout: Multiclass label indicating dropout status.
                   For example:
                     0: No Dropout
                     1: Partial/At-Risk
                     2: Complete Dropout
...
def train_neural_network_multiclass_classifier(df):
    """
    Train a Neural Network classifier (MLPClassifier) for multiclass classification using student data.
    
    The classifier uses the following features:
        - attendance
        - prev_gpa
    to predict the dropout status, which is a multiclass label (e.g., 0, 1, 2).
    
    A pipeline is used to first scale the data and then train the MLPClassifier.
    ...
    # Extract the multiclass target labels.
    y = df['dropout'].values
    
    # Create a pipeline that first scales the data and then trains a Neural Network classifier.
    # The MLPClassifier automatically handles multiclass classification.
    model = make_pipeline(
        StandardScaler(),
...
def predict_dropout_status(model, new_data):
    """
    Predict the multiclass dropout status for new student data using the trained Neural Network classifier.
    ...
    Returns:
        predictions (list): Predicted dropout statuses (multiclass labels) for each new data point.
    """
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
    # Step 2: Train the Neural Network classifier for multiclass classification using the fetched data.
    # -------------------------------------------------------------------------
    print("\nTraining Neural Network classifier for multiclass classification...")
    multiclass_model = train_neural_network_multiclass_classifier(df)
    ...
    predictions = predict_dropout_status(multiclass_model, new_students)
    
    # Display the predictions in a user-friendly format.
    for i, prediction in enumerate(predictions):
        print(f"Student {i+1}: Predicted dropout status - {prediction}")
